...
hld_bos_LQ <- read_csv("Data/hld_bos_LQ.csv", show_col_types = FALSE)
sun_bos_LQ <- read_csv("Data/sun_bos_LQ.csv", show_col_types = FALSE)
har_bos_LQ <- read_csv("Data/har_bos_LQ.csv", show_col_types = FALSE)
```

# 1 Sysselsetting i ulike næringer, etter arbeidssted

**Antall personer som jobber i kommunen**
...
Figur 13 og 14 viser den totale utviklingen i Odda (2019-) og Ullensvang frem til 2020. Vi ser at Odda har hatt en jevn reduksjon i sysselsetningen totalt. Dette kan ha noe med at Odda har hatt en generelt aldrende befolkning siste årene. En ser at Ullensvang har en jevn utvikling frem til 2019, før den får eksplosiv vekst. Denne veksten kommer av sammenslåingen av Ullensvang, Odda og Jondal i 2019.

For å få en bedre oversikt over utviklingen innenfor de mest relevante næringene, så ser vi på figur 16.
...
I figur 16 så ser en klart at det har vert en nedgang innen alle sektorer i Odda, med untakt av industri som har sett en vekst fra 2016 frem til 2019 før sammenslåingen. Ser videre at Ullensvang har hatt ein vekst innenfor Helse- og Sosialetjenester, men resten av sektorene har hatt en reduksjon helt fram til sammenslåingen i 2019.
...
**Hvor mange som bur i kommunen som jobber innenfor næring x**

Det neste vi ønsker å gjøre er å se bostedssituasjonen til arbeidstakere. Det vi ønsker å se på her da er hvor mange som jobber innenfor en sektor, f.eks. industri, som bor i regionen.

Den første regionen vi velger å se på i denne sammenhengen er Haugalandet. 
...
Vi ser allerede ut ifra figur 17 to faktorer. Det ene er at antall bosatte på Haugalandet som jobber innenfor næringene er høyere enn antall personer som jobber i regionen. Dette tyder på at det er en god del pendling til og fra regionen. Det andre vi kan se ut ifra figur 17 er at Haugalandet totalt sett hadde en vekst fra 2009 frem til 2014, og opplevd vekste fra 2016 til 2019, og ser ut til å gå gjennom en ny reduksjon frem mot i dag. 

For å få et bedre overblitt hvordan utviklingen er på et kommunalt nivå så ser vi på figur 18.
...
Figur 18 viser her totale utviklingen på kommunalt nivå i Haugalandsregionen. Vi ser desverre her at på grunn av at Haugesund og Karmøy er såpass store og dominante at det blir uoversiktlig å få med seg utviklingen til dei mindre kommunane. Figur 18 gjer oss ein god oversikt over regionen, men vi velger dermed sjå videre på figur 19 og 20 som gjer ein mykje bedre oversikt over utviklinga på kommunalt nivå.
...
Vi ser nå utviklinga til kommunane i regionen mye tydeligere, og vi kan med en gang se at flere av kommunene har sett en jevn reduksjon totalt sett. Litt interessant å bemerke at de eneste to kommunene som har sett jevn vekst siden 2008 er Bokn og Tysvær. Karmøy såg en kraftig vekst fra 2010 til 2014, men har etter dette sett en kraftig reduksjon. Å se på totale utviklingen i regionen gir oss ikke nødvendigvis all nødvendig informasjon, så vi velger å filtrere videre ned til de 6 mest relevante næringene i regionen på kommune nivå.
...
Figurene 21 og 22 gir oss en oversikt over utviklingen i regionen på kommunalt nivå, men nå med hensyn på de 6 mest relevante næringene. Vi ser ut ifra figurene at alle kommunene i regionen har en ting til felles, og det er en reduksjon i industrien og vekste i helse- og sosialetjenester. Eneste kommunen som skiller seg litt ut her, er Bokn som har sett en svak vekst i industri siste 4 årene.
...
- Eir_Er_industri = Samlet sysselsetting i sektor Industri i region Haugalandet
- Eir_Er_Jordbruk = Samlet sysselsetting i region Haugalandet
- Eir_Er_Undervisning = Samlet nasjonal sysselsetting i sektor Industri
- Eir_Er_Helse = Samlet nasjonal sysselsetting

```{r hld_industri}
hld_bos_LQ <- hld_bos_LQ %>% 
  # Slik finner vi samlet sysselsetting i Industri i Haugelandet for år 2008-2020
  mutate(Eir_Er_industri = Totalt_Reg_Industri/Totalt_Reg_Næringer,
         Eir_Er_Jordbruk = Totalt_Reg_Primærnæring/Totalt_Reg_Næringer,
         Eir_Er_Undervisning = Totalt_Reg_Undervisning/Totalt_Reg_Næringer,
         Eir_Er_Helse = Totalt_Reg_Helsetjenester/Totalt_Reg_Næringer)
```
...
- Eir_Er_industri = Samlet sysselsetting i sektor Industri i region Sunnhordland
...
### Hardanger 2008-2020
- Eir_Er_industri = Samlet sysselsetting i sektor Industri i region Hardanger
...
NÅ MÅ VI HA NASJONALT DATASETT FOR Å DELE VERIDENE PÅ NASJONAL SYSSELSETTING OG FÅ LOKALISERINGSKVOTIENTEN
...
## Lokaliseringskvotienter for enkeltkommuner

```{r}
hau08 <- syssel_bos %>% 
  filter(aar == "2008", knavn == "Haugesund")

(hau08$Industri/hau08$`Alle Næringer`)
2289/17611
```

$\frac{Eir}{Er}=\frac{2289}{17611}=0.1299756$
Vi ser at industri utgjør 13% av sysselsettingen i Haugesund i år 2008. Dette må vi dividere med $\frac{Nasjonal\,sysselsetting\,i\, industri}{Nasjonal\,sysselsetting}$
Industrien i Haugesund har redusert både i andel, og faktisk størrelse. 
...
$\frac{Eir}{Er}=\frac{1905}{18137}=0.1050339$
Vi ser at industri utgjør 10% av sysselsettingen i Haugesund i år 2020. Dette må vi dividere med $\frac{Nasjonal\,sysselsetting\,i\, industri}{Nasjonal\,sysselsetting}$

```{r}
kar08 <- syssel_bos %>% 
  filter(aar == "2008", knavn == "Karmøy")
```

```{r}
(kar08$Industri/kar08$`Alle Næringer`)
3694/20182
```

$\frac{Eir}{Er}=\frac{2289}{17611}=0.1299756$
Vi ser at industri utgjør 18% av sysselsettingen i Karmøy i år 2008. Dette må vi dividere med $\frac{Nasjonal\,sysselsetting\,i\, industri}{Nasjonal\,sysselsetting}$
Industrien i Karmøy har minket både i andel, og faktisk størrelse. 

```{r}
kar20 <- syssel_bos %>% 
  filter(aar == "2020", knavn == "Karmøy")
```

```{r}
(kar20$Industri/kar20$`Alle Næringer`)
2725/20063
```

$\frac{Eir}{Er}=\frac{1905}{18137}=0.1050339$
Vi ser at industri utgjør 13,5% av sysselsettingen i Karmøy i år 2020. Dette må vi dividere med $\frac{Nasjonal\,sysselsetting\,i\, industri}{Nasjonal\,sysselsetting}$
